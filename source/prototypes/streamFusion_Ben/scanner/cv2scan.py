...
def getPlayableArray(img):
    ...
    # pippoRGBA2.show()
    cv2.imwrite(
        './source/prototypes/streamFusion_Ben/output/imgAdaptiveThre.png', imgAdaptiveThre)
    ...
    with open('./source/prototypes/streamFusion_Ben/output/mapArray.txt', 'w') as f:
        f.writelines(repr(iar))

    # pippoRGBA2 = Image.fromarray(np.array(newImg).astype('uint8'), mode='RGBA')
    # pippoRGBA2.show()
    cv2.imwrite(
        './source/prototypes/streamFusion_Ben/output/newImg.png', np.array(newImg))
